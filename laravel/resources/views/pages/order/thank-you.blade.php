@extends('layouts.default')
@section('head')
    <link rel="stylesheet" href="/styles/thank-you-style.css">
@stop
@section('content')
<div class="container align-middle">
    <article class="container align-middle pt-4 bg-white shadow-sm rounded-extra mt-5 mb-3 p-3">
        <h1 class="text-center pb-3"><b>Ďakujeme za vašu objednávku!</b></h1>
        <h3 class="text-center ">Vaša objednávka bude spracovaná a odoslaná v čo najkratšom čase.</h3>
        <h3 class="text-center text-black-50">objednávka č. {{ sprintf("%08d", $order_id) }}</h3>
        <div class="d-flex justify-content-center p-2">
            <a href="/"><img class="img-fluid thank-you-logo" src="../../images/logo/logo.svg" width="200"
                             height="160" alt="LOGO"/></a>
        </div>
    </article>
</div>
@stop
